...
export function Summary() {
  ...
  const firdtDayOfWeek = dayjs().startOf("week").format("D MMM");
  const lastDayOfWeek = dayjs().endOf("week").format("D MMMM");

  const compltetedPercentage = Math.round(
    ...
    <div className="py-10 max-w-[480px] px-5 mx-auto flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3">
        <Progress value={8} max={15}>
          <ProgressIndicator style={{ width: `${compltetedPercentage}%` }} />
        </Progress>
        ...
        <div className="flex items-center justify-between text-xs text-zinc-400">
          ...
          <span>{compltetedPercentage}%</span>
        </div>
